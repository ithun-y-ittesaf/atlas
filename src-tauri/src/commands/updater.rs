...
//!
//! On non-macOS platforms (Windows, Linux) Atlas doesn't yet ship a
//! signed/verifiable update artifact, so this module falls back to an inert
//! stub below ([`imp` for `not(target_os = "macos")`]) that exposes the same
//! commands/state shape but never finds an update — auto-update is disabled
//! outright rather than half-implemented. Frontend callers see a permanently
//! "idle" [`UpdaterSnapshot`] and an "unavailable" [`UpdateStatus`], not an error.

use serde::Serialize;

/// Result of an explicit "check for updates" call.
...
mod imp {
    ...
    use super::{UpdateStatus, UpdaterSnapshot, CURRENT_VERSION};
...
/// Inert stub on non-macOS platforms — Atlas doesn't yet ship a signed update
/// artifact for Windows/Linux, so every entry point here is a no-op / permanent
/// "idle" state rather than a partial implementation. See the sibling
/// `#[cfg(target_os = "macos")]` `imp` module for the real update flow this
...
mod imp {
    ...
    /// No download/staging ever happens on this platform, so there's nothing to
    /// track — a unit struct is enough to satisfy `tauri::State<UpdaterState>`.
    #[derive(Default)]
    pub struct UpdaterState;
    ...
    /// Manual "Check for updates" — always reports no update available.
    #[tauri::command]
    pub async fn update_check_now(_app: AppHandle) -> Result<UpdateStatus, String> {
        Ok(UpdateStatus {
            available: false,
            version: None,
            current_version: CURRENT_VERSION.to_string(),
    ...
    /// "Restart now" — there is never a staged update to apply here.
    #[tauri::command]
    pub async fn update_apply(_app: AppHandle) -> Result<(), String> {
        Err("auto-update isn't available on this platform yet".into())
    }

    /// Current updater state for UI hydration on mount — always idle.
    #[tauri::command]
    pub fn update_state(_app: AppHandle, _state: tauri::State<'_, UpdaterState>) -> UpdaterSnapshot {
        UpdaterSnapshot {
            phase: "idle".into(),
            version: None,
            current_version: CURRENT_VERSION.to_string(),
    ...
    /// Persist a "don't prompt for this version again" choice — no-op, since we
    /// never prompt on this platform.
    #[tauri::command]
    pub fn update_ignore(
        _version: String,
        _app: AppHandle,
        _state: tauri::State<'_, crate::state::AppStateHandle>,
    ) -> Result<(), String> {
        Ok(())
    }
}
